...
export async function _getAllPending(request:any, h:any):Promise<any>{
    const { query } = request;
    query.paginationPage = query.paginationPage || 1;
    query.paginationPerPage = query.paginationPerPage || 10;
    try {
...
export async function _applause(request:any, h:any):Promise<any>{
    const { id } = request.params;
    let originalModel;
    ...
    try {
        ...
        await Model.updateOne({_id : id }, {$set: { applauses: originalModel.applauses+1 } });
        return h.response({_id: id}).code(200);
    } catch (error) {
        console.log(error);
        await Model.updateOne(
